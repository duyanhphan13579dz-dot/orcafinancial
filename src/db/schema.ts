...
export const financialStatements = pgTable(
  "financial_statements",
  {
    ...
    /** Phase 2: synthetic rows must never win over filings in queries */
  ...
  (t) => [
    ...
    index("fs_stmt_synthetic_idx").on(t.symbol, t.isSynthetic),
    index("fs_stmt_priority_idx").on(t.symbol, t.sourcePriority),
  ],
);
...
export const financialNormalizedFacts = pgTable(
  "financial_normalized_facts",
  {
    ...
    /** Phase 2: canonical storage unit — absolute VND preferred */
    canonicalUnit: varchar("canonical_unit", { length: 20 }).notNull().default("VND"),
    ...
    verificationStatus: varchar("verification_status", { length: 20 }).notNull().default("unverified"),
    isSynthetic: boolean("is_synthetic").notNull().default(false),
    sourcePriority: integer("source_priority").notNull().default(40),
    dataVersion: varchar("data_version", { length: 20 }).notNull().default("v1"),
  ...
  (t) => [
    uniqueIndex("fs_normalized_fact_uq").on(t.symbol, t.statementType, t.period, t.fiscalYear, t.reportScope, t.source),
    index("fs_normalized_symbol_idx").on(t.symbol, t.fiscalYear, t.period),
    index("fs_normalized_priority_idx").on(t.symbol, t.sourcePriority),
  ],
);
