...
export const users = pgTable("users", {
  ...
  passwordHash: varchar("password_hash", { length: 255 }), // null nếu đăng nhập Google
...
export const userPreferences = pgTable("user_preferences", {
  ...
  // Appearance
  theme: varchar("theme", { length: 20 }).notNull().default("dark"), // light | dark | system
  accentColor: varchar("accent_color", { length: 20 }).notNull().default("#00d4ff"),
  language: varchar("language", { length: 10 }).notNull().default("vi"),
  fontScale: varchar("font_scale", { length: 10 }).notNull().default("md"), // sm | md | lg
...
// Login sessions — one row per device/browser session (linked to a refresh token).
...
// Audit log — security-relevant user actions.
export const auditLogs = pgTable("audit_logs", {
  id: uuid("id").primaryKey().defaultRandom(),
  userId: uuid("user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
  action: varchar("action", { length: 60 }).notNull(), // login | logout | change_password | update_profile | ...
  metadata: jsonb("metadata").$type<Record<string, unknown>>(),
  ipAddress: varchar("ip_address", { length: 60 }),
  userAgent: varchar("user_agent", { length: 400 }),
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
