...
export function markStale(kind: string, symbol: string | null, reason: string) {
  ...
  logger.warn("data_marked_stale", { kind, symbol, since: new Date().toISOString(), reason });
}
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        log.warn("http_non_ok", { url, method: rest.method ?? "GET", status: res.status, attempt, durationMs, retryable: cls.retryable });
      ...
      log.debug("http_ok", { url, method: rest.method ?? "GET", status: res.status, attempt, durationMs });
      return res;
    } catch (err) {
      clearTimeout(timer);
      const durationMs = Date.now() - started;
...
export async function readJsonSafe<T = unknown>(res: Response, provider: string, url: string): Promise<T> {
  ...
  try {
    ...
    forProvider(provider).error("json_parse_failed", {
      url,
...
export async function mapPool<T, R>(
  ...
  maybeFn?: (item: T, index: number) => Promise<R>,
): Promise<PromiseSettledResult<R>[]> {
  let concurrency: number;
  let fn: (item: T, index: number) => Promise<R>;
  if (typeof concurrencyOrFn === "function") {
    concurrency = CONNECTOR_CONFIG.quoteConcurrency;
    fn = concurrencyOrFn;
  } else {
    concurrency = concurrencyOrFn;
    fn = maybeFn as (item: T, index: number) => Promise<R>;
  }
