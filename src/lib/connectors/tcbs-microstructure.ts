import { isTcbsMockEnabled } from "@/lib/connectors/tcbs-mock";

export type MarketDataStatus = "live" | "delayed" | "stale" | "unavailable";
export interface DepthLevel { price: number; volume: number; orders: number; }
...
export function tcbsMockMicrostructure(symbol: string, close: number, now = Math.floor(Date.now() / 1000)): StockMicrostructureSnapshot {
  const seed = hash(symbol);
  const tick = close >= 100 ? 0.1 : close >= 10 ? 0.05 : 0.01;
  const step = tick * (1 + (seed % 3));
  const baseVolume = 500 + (seed % 5000);
  const bids = Array.from({ length: 5 }, (_, i) => ({ price: round(close - step * (i + 1)), volume: baseVolume + ((seed + i * 613) % 4200), orders: 8 + ((seed + i * 11) % 68) }));
  const asks = Array.from({ length: 5 }, (_, i) => ({ price: round(close + step * (i + 1)), volume: baseVolume + ((seed + i * 877) % 3900), orders: 7 + ((seed + i * 13) % 64) }));
