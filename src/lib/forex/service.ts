...
let syncPromise: Promise<{ source: string; saved: number; timestamp: Date; durationMs: number }> | null = null;

const OHLCV_SOFT_TTL: Record<string, number> = { "1m": 20_000, "5m": 45_000, "15m": 90_000, "1h": 180_000, "4h": 600_000, "1d": 1_800_000 };
const OHLCV_HARD_TTL: Record<string, number> = { "1m": 90_000, "5m": 180_000, "15m": 360_000, "1h": 900_000, "4h": 2_700_000, "1d": 7_200_000 };
  ...
  for (let i = 0; i < FOREX_PAIRS.length; i += chunk) {
    await Promise.all(
      FOREX_PAIRS.slice(i, i + chunk).map((p) =>
        db.insert(forexPairs).values({
          symbol: p.symbol, name: p.name, category: p.category,
          baseCurrency: p.baseCurrency, quoteCurrency: p.quoteCurrency,
          yahooSymbol: p.yahooSymbol, source: "multi-source",
        }).onConflictDoUpdate({
          target: forexPairs.symbol,
          set: { name: p.name, category: p.category, baseCurrency: p.baseCurrency, quoteCurrency: p.quoteCurrency, yahooSymbol: p.yahooSymbol, updatedAt: new Date() },
        }),
      ),
    );
  }
...
export async function syncForexPrices() {
  ...
  syncPromise = (async () => {
    ...
    for (let i = 0; i < quotes.length; i += 12) {
      await Promise.all(quotes.slice(i, i + 12).map(async (q) => {
        const pair = by.get(q.symbol)!;
        await db.insert(forexPrices).values({
          pairId: pair.id, price: q.price, bid: q.bid, ask: q.ask,
          change: q.change, changePercent: q.changePercent, source: snapshot.source, timestamp,
        }).onConflictDoUpdate({
          target: [forexPrices.pairId, forexPrices.timestamp],
          set: { price: q.price, bid: q.bid, ask: q.ask, change: q.change, changePercent: q.changePercent, source: snapshot.source, createdAt: new Date() },
        });
        saved++;
      }));
    }
    log.info("forex_prices_synced", { source: snapshot.source, saved, durationMs: Date.now() - started });
    return { source: snapshot.source, saved, timestamp, durationMs: Date.now() - started };
  ...
  const r = await db.execute(sql`
    WITH latest AS (
      SELECT DISTINCT ON (p.pair_id)
        ...
        f.base_currency AS "baseCurrency", f.quote_currency AS "quoteCurrency",
        p.price, p.bid, p.ask, p.change, p.change_percent AS "changePercent", p.source, p.timestamp
      FROM forex_pairs f JOIN forex_prices p ON p.pair_id = f.id
...
export async function getForexPair(symbol: string) {
  const [pair] = await db.select().from(forexPairs).where(eq(forexPairs.symbol, symbol.toUpperCase())).limit(1);
  if (!pair) return null;
  const [price] = await db.select().from(forexPrices).where(eq(forexPrices.pairId, pair.id)).orderBy(desc(forexPrices.timestamp)).limit(1);
  return { pair, price };
}

type DbOhlcv = { bars: Ohlcv[]; source: string; newestMs: number };

async function readOhlcvFromDb(pairId: string, timeframe: string, limit: number): Promise<DbOhlcv | null> {
  const rows = await db.select().from(forexOhlcv)
    .where(and(eq(forexOhlcv.pairId, pairId), eq(forexOhlcv.timeframe, timeframe)))
    .orderBy(desc(forexOhlcv.time)).limit(limit);
...
async function persistBars(pairId: string, timeframe: string, bars: Ohlcv[], source: string) {
  ...
  for (let i = 0; i < bars.length; i += chunk) {
    await Promise.all(bars.slice(i, i + chunk).map((b) =>
      db.insert(forexOhlcv).values({
        pairId, timeframe, time: new Date(b.time * 1000),
...
/** Stale-while-revalidate: soft TTL = instant DB; hard TTL = must network. */
export async function syncForexOhlcv(symbol: string, timeframe: string, limit = 300) {
  let found = await getForexPair(symbol);
  if (!found) { await initializeForexPairs(); found = await getForexPair(symbol); }
...
export async function runForexAnalysis(symbol: string, timeframe = "1h") {
  ...
  void db.insert(forexAnalysis).values({
    pairId: pair.id, timeframe,
    technicalSignals: a.indicators,
    patterns: { candlestick: a.candlestickPatterns, chart: a.chartPatterns },
    recommendation: a.recommendation,
    entryPrice: a.entryPrice, stopLoss: a.stopLoss, takeProfit: a.takeProfit,
    confidence: a.confidence, reason: a.reasons.join("; "), timestamp: new Date(),
...
/** One-shot first-paint: pair + price + bars + analysis. */
export async function getForexDetailBundle(symbol: string, timeframe = "1h", limit = 300) {
  ...
  const [detail, ohlcv, analysis] = await Promise.all([
    (async () => { await ensureForexFresh(8000); return getForexPair(sym); })(),
  ...
  return { pair: detail.pair, price: detail.price, bars: ohlcv.bars, timeframe, source: ohlcv.source, analysis };
}
