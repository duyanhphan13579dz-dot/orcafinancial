 ...
 * Explicit row types.
 *
 * Using ReturnType<typeof db.select> here produces a Drizzle
 * query-builder type rather than the selected row array.
 */
...
let pairsCache:
  | CacheEntry<ForexPairRow[]>
  | null = null;

let latestPricesCache:
  | CacheEntry<unknown[]>
  | null = null;
...
export async function initializeForexPairs() {
  if (
    ...
    return pairsCache!.value.length;
  }
...
export async function syncForexPrices() {
  ...
  syncPromise = (async () => {
    ...
    /*
     * Only initialize when the pair cache is absent.
     * Previously this was executed on every price refresh.
     */
    if (
      !cacheIsFresh(
        pairsCache,
...
export async function ensureForexFresh(
  maxAgeMs = 10_000,
) {
  if (
    cacheIsFresh(
...
export async function listForexPairs(
  opts: {
    category?: string;
    search?: string;
  } = {},
 ...
 * pair + live price + chart + analysis.
 */
