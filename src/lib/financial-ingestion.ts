...
function sourcePriorityOf(source: string): number {
  if (source === "tcbs") return 10;
  if (source === "vietstock") return 20;
  if (source === "cafef") return 30;
  return 40;
}
...
export async function loadPreferredQuarterlyFinancials(
  ...
  // Phase 0: never fall back to synthetic generation.
  return {
    quarters: [],
    source: "vietstock",
    providerBacked: false,
    warnings: ["Chưa có đủ normalized facts đã xác minh từ Vietstock/CafeF/TCBS. Không dùng synthetic fallback (Verified Financial Data policy)."],
  };
}
...
export async function loadPreferredFinancialRecords(
  ...
  statementType: StatementType,
  limit = 8,
): Promise<{ records: import("@/lib/stock-intelligence/financial-source").RawFinancialRecord[]; source: "tcbs" | "vietstock" | "cafef"; providerBacked: boolean }> {
  await ensureFinancialIngestionTables();
  const rows = await db
    .select()
    .from(financialNormalizedFacts)
