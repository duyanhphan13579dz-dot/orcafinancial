...
function sourcePriorityOf(source: string): number {
  return sharedSourcePriorityOf(source);
}
    ...
    const facts = Array.isArray(record.facts)
      ? record.facts.flatMap((fact) => {
          ...
          return [{
            statementType,
            period: factPeriod,
            fiscalYear: Number(factPeriod.slice(-4)),
            reportScope: (text(f.reportScope) as SourceFact["reportScope"]) ?? "unknown",
            currency: text(f.currency),
            unit: text(f.unit),
            periodEnd: text(f.periodEnd),
            filingDate: text(f.filingDate),
            data: asRecord(f.data) ?? {},
            evidence: asRecord(f.evidence) as SourceFact["evidence"] | undefined,
          }];
        })
      : undefined;
    return [{
    ...
    facts: (
      ...
      .map(([statementType, data]) => ({
        ...
        evidence: Object.fromEntries(
          Object.entries(data).map(([key, value]) => [key, { sourceValue: value, normalizedValue: value }]),
        ),
      })),
  }));
}
...
class VndirectFinancialAdapter implements SourceAdapter {
  ...
  async fetch(symbol: string, limit: number): Promise<{ documents: SourceDocument[]; warnings: string[] }> {
    try {
      ...
      return { documents: [], warnings: [`vndirect: ${e instanceof Error ? e.message : String(e)}`] };
    }
  }
}

class VietstockFinancialAdapter implements SourceAdapter {
  source = "vietstock" as const;
  ...
  async fetch(symbol: string, limit: number): Promise<{ documents: SourceDocument[]; warnings: string[] }> {
    try {
      ...
      return { documents: [], warnings: [`vietstock: ${e instanceof Error ? e.message : String(e)}`] };
    }
  }
}

class CafefFinancialAdapter implements SourceAdapter {
  ...
  async fetch(symbol: string, limit: number): Promise<{ documents: SourceDocument[]; warnings: string[] }> {
    try {
      ...
      return { documents: [], warnings: [`cafef: ${e instanceof Error ? e.message : String(e)}`] };
    }
  }
}
...
async function persistDocument(document: SourceDocument): Promise<number> {
  ...
  const existing = await db
    .select({ id: financialSourceDocuments.id })
    .from(financialSourceDocuments)
    .where(eq(financialSourceDocuments.documentHash, documentHash))
    .limit(1);
...
export async function ingestSourceDocuments(documents: SourceDocument[]): Promise<IngestionResult> {
    ...
    for (const fact of document.facts ?? []) {
      ...
      await db
        ...
        .values({
          ...
          qualityIssues: [],
          isSynthetic: false,
          sourcePriority: sourcePriorityOf(document.source),
          canonicalUnit: "VND",
          data: fact.data,
        })
        .onConflictDoUpdate({
  ...
  return {
    ...
    symbols: [...new Set(documents.map((document) => document.symbol))],
    sources: ["vndirect"],
...
export async function loadPreferredQuarterlyFinancials(
  symbol: string,
  limit = 4,
): Promise<{ quarters: import("@/lib/financial-statements").FinancialQuarter[]; source: string; providerBacked: boolean; warnings: string[] }> {
  await ensureFinancialIngestionTables();
  const rows = await db
    .select()
    .from(financialNormalizedFacts)
    .where(eq(financialNormalizedFacts.symbol, symbol))
    .orderBy(desc(financialNormalizedFacts.fiscalYear), desc(financialNormalizedFacts.period), desc(financialNormalizedFacts.normalizedAt))
    .limit(Math.min(120, Math.max(12, limit * 12)));
  const grouped = new Map<string, Partial<import("@/lib/financial-statements").FinancialQuarter> & { sources: string[] }>();
  for (const row of rows.filter(
    (item) => item.qualityStatus === "accepted" && item.verificationStatus === "verified" && /^Q[1-4]\/\d{4}$/i.test(item.period),
  )) {
  ...
  const complete = [...grouped.values()]
    .filter((item): item is import("@/lib/financial-statements").FinancialQuarter & { sources: string[] } =>
      Boolean(item.income && item.balance && item.cashflow && item.fiscalYear && item.quarter),
    )
  ...
  if (complete.length > 0) {
    return {
      quarters: complete,
      source: [...new Set(complete.flatMap((item) => item.sources))].join(","),
      providerBacked: true,
      warnings: [],
    };
  }
  return {
    ...
    warnings: ["Chưa có đủ normalized facts đã xác minh từ VnDirect/Vietstock/CafeF. Không dùng synthetic fallback (Verified Financial Data policy)."],
  };
}

export async function loadPreferredFinancialRecords(
  symbol: string,
  statementType: StatementType,
  limit = 8,
  ...
  const rows = await db
    ...
    .where(eq(financialNormalizedFacts.symbol, symbol))
    .orderBy(desc(financialNormalizedFacts.fiscalYear), desc(financialNormalizedFacts.period), desc(financialNormalizedFacts.normalizedAt))
    .limit(Math.min(60, Math.max(3, limit * 3)));
  ...
  const records = accepted.slice(0, limit).map((row) => ({
    period: row.period,
    fiscalYear: row.fiscalYear,
    reportedCurrency: row.currency,
    data: row.data as Record<string, unknown>,
    source: row.source,
    retrievedAt: row.normalizedAt.toISOString(),
  ...
  return { records: [], source: "vietstock", providerBacked: false };
}
...
export async function getFinancialSourceEvidence(symbol: string, limit = 12) {
  ...
  const facts = await db
    .select({
      documentId: financialNormalizedFacts.documentId,
      qualityStatus: financialNormalizedFacts.qualityStatus,
      verificationStatus: financialNormalizedFacts.verificationStatus,
    })
    .from(financialNormalizedFacts)
    .where(eq(financialNormalizedFacts.symbol, symbol))
    .limit(100);
