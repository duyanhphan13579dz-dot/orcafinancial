...
function parseSentimentJson(raw: string): { score: number; label: string; confidence: number; rationale: string } | null {
  const trimmed = raw.trim();
  // Extract JSON object even if model wraps it in markdown fences
...
/**
 * Hybrid sentiment for a symbol/coin:
 * 1. Rule-engine baseline (always available, no cost).
 * 2. Optional LLM refinement when headlines exist and a free-tier key is set.
 * 3. Blend: 0.4 * rule + 0.6 * llm when LLM succeeds; else pure rule.
 */
export async function scoreSentimentHybrid(
  ...
  if (opts.forceRuleOnly || texts.length === 0 || process.env.LLM_SENTIMENT_DISABLED === "1") {
    return {
      ...
      rationale: texts.length > 0 ? "Rule-based NLP tiếng Việt / English lexicon." : "Không có tin để chấm.",
      source: "rule-engine",
    };
  }
  ...
  try {
    const llm = await chatWithFallback(
      ...
      { maxTokens: 400, temperature: 0.1, timeoutMs: 18_000 },
    );
    ...
    if (!llm) {
      return {
        ...
        label: ruleLabel,
        confidence: 0.55,
        rationale: "Rule-based NLP (LLM không khả dụng).",
        source: "rule-engine",
      };
    }
    ...
    if (!parsed) {
      logger.warn("sentiment_llm_parse_failed", { symbol, snippet: llm.text.slice(0, 120) });
      return {
        score: Number(ruleScore.toFixed(3)),
        label: ruleLabel,
        confidence: 0.55,
        rationale: "Rule-based NLP (LLM trả JSON không hợp lệ).",
        source: "rule-engine",
      };
    }

    // Blend rule + LLM to reduce hallucination risk
    ...
    return {
      ...
      label: ruleLabel,
      confidence: 0.55,
      rationale: "Rule-based NLP (lỗi LLM).",
      source: "rule-engine",
    };
  }
}

/** Agent narrative helper — thin wrapper around chatWithFallback. */
export async function agentNarrative(userQuestion: string, contextBlock: string) {
  return chatWithFallback(
    [
      { role: "system", content: AGENT_SYSTEM_PROMPT },
      {
        role: "user",
        content: `DỮ LIỆU REAL-TIME TỪ DATA ENGINE:\n${contextBlock}\n\nCÂU HỎI: ${userQuestion}`,
      },
    ],
    { maxTokens: 1600, temperature: 0.35, timeoutMs: 30_000 },
  );
}
