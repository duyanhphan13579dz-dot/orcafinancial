...
function StatusLamp({ state }: { state: "live" | "idle" | "error" | "pending" }) {
  ...
  return (
    <span className="inline-flex items-center gap-2">
      ...
      />
      <span className="font-mono text-[10px] tracking-[0.25em] uppercase" style={{ color: map.c }}>
        {map.label}
      </span>
    </span>
  );
...
export default function ReportsPage() {
  ...
  const doTrigger = async (type: "morning" | "summary", silent = false) => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/v1/reports/trigger/${type}`, { method: "POST" });
      const j = await res.json().catch(() => ({}));
      if (!res.ok) {
        const msg = j?.error || j?.message || `Tạo báo cáo thất bại (HTTP ${res.status})`;
        if (!silent) setError(String(msg));
        return;
      }
      const html = j?.data?.html as string | undefined;
      const date = (j?.data?.date as string) || todayKey;
  ...
  const openPreview = async (r: ReportRow) => {
    try {
      ...
      if (!res.ok) {
        setError(j?.error || `Không tải được báo cáo ${r.type}`);
        return;
      }
      ...
      >
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <div
                  ...
                  style={{ color: accent }}
                >
                  {type === "morning" ? "07:30 ICT · Mon–Fri" : "15:15 ICT · Mon–Fri"}
                </div>
                <h2 className="font-display text-xl md:text-2xl font-extrabold text-white tracking-tight leading-tight">
                  {title}
                </h2>
              </div>
            </div>
        ...
        {history.length > 0 && (
          <div className="relative mt-5 pt-4 border-t border-[#1a3558]/70">
            <div className="font-mono text-[9px] tracking-[0.25em] uppercase text-slate-500 mb-2">
                  ...
                  <button
                    ...
                    className={`group relative rounded border px-2 py-1 text-[10px] font-mono tabular-nums transition-all hover:-translate-y-0.5 ${
                      isToday
                        ...
                        : "border-[#1a3558] text-slate-400 hover:border-[#00d4ff]/60 hover:text-white"
                    }`}
                    title={r.title}
                  >
  ...
  return (
    <ProtectedPage featureName="báo cáo hàng ngày">
      <div className="space-y-6">
        <header className="relative overflow-hidden panel p-6 md:p-8 scanlines">
          ...
          <div className="relative flex flex-wrap items-end justify-between gap-6">
            ...
            <div className="text-right">
              ...
              <div
                className={`mt-2 inline-block rounded-full px-2.5 py-0.5 font-mono text-[10px] tracking-widest ${
                  isWeekday
        ...
        {error && (
          <div className="panel border-rose-700 bg-rose-950/30 p-3 text-sm text-rose-300 flex items-center justify-between gap-3">
            <span className="break-words">{error}</span>
            <button onClick={() => setError(null)} className="text-rose-400 hover:text-white text-xs shrink-0">
              đóng
            </button>
          </div>
        )}
                    ...
                    <button
                      ...
                      className={`w-full text-left rounded border p-3 transition-all hover:-translate-y-0.5 ${
                        isToday
                          ...
                          : "border-[#1a3558] bg-[#0a1d33]/40 hover:border-[#00d4ff]/40"
                      }`}
                    >
                      <div className="flex items-center justify-between flex-wrap gap-2">
                        <div className="flex items-center gap-3">
                          ...
                          {isToday && (
                            <span className="font-mono text-[9px] tracking-widest text-[#00d4ff] uppercase border border-[#00d4ff]/40 rounded px-1.5 py-0.5">
                              Hôm nay
                            </span>
                          )}
                        </div>
        ...
        {preview && (
          ...
          >
            <div
              className="no-print flex flex-wrap items-center justify-between gap-3 mb-3"
            ...
            >
              ...
              <div className="flex gap-2 flex-wrap">
                ...
                <button onClick={doOpenTab} className="btn-orca-outline text-xs hidden sm:inline-flex">
                  Mở tab mới
                </button>
                <button onClick={() => setPreview(null)} className="btn-orca-ghost">
                  Đóng ✕
                </button>
              </div>
            </div>
            <div
              className="flex-1 rounded-lg overflow-hidden bg-white border border-[#1a3558]"
              onClick={(e) => e.stopPropagation()}
            >
              <iframe
                ref={iframeRef}
