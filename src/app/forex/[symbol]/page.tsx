  ...
  return (
    <div className="mx-auto max-w-7xl space-y-3 sm:space-y-4">
          ...
          <div className="min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5 text-[10px] text-slate-500">
              <span className="inline-flex items-center gap-1">
                <span className={`inline-block h-1.5 w-1.5 rounded-full ${freshnessDot(freshness)}`} />
                <span className="font-semibold text-slate-300">{freshness ?? "—"}</span>
              </span>
              {ageLabel(ageMs) && <span>{ageLabel(ageMs)}</span>}
              {spreadPips != null && Number.isFinite(spreadPips) && (
                <span className="hidden sm:inline">Spr {spreadPips.toFixed(1)}p</span>
              )}
              {fx?.session ? <span className="hidden md:inline">· {fx.session.label}</span> : null}
            </div>
          </div>
        </div>
      ...
      <div className="flex flex-col gap-3 lg:grid lg:grid-cols-3 lg:gap-4">
        <div className="order-1 space-y-2.5 lg:order-2 lg:col-span-1">
          {a ? (
            ...
            <div className="panel space-y-3 p-4">
              ...
              <p className="text-[10px] text-slate-500">
                {analysisLoading || bundleLoading ? "Đang tính tín hiệu…" : "Chưa có phân tích"}
              </p>
            </div>
          )}
          <ForexScalpingPanel
        ...
        <div className="panel order-2 p-2.5 sm:p-3 lg:order-1 lg:col-span-2">
          <div className="mb-2 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <h2 className="text-sm font-semibold text-white sm:text-base">Chart</h2>
          ...
          <div className="mb-2 flex flex-wrap gap-1.5">
            {([["EMA", showEma, setShowEma], ["BB", showBb, setShowBb], ["RSI", showRsi, setShowRsi], ["MACD", showMacd, setShowMacd]] as const).map(([label, on, set]) => (
              <button
                key={label}
                type="button"
                onClick={() => set(!on)}
                className={`rounded border px-2 py-1 text-[10px] ${
                  on ? "border-[#00d4ff]/50 bg-[#00d4ff]/10 text-[#00d4ff]" : "border-slate-700 text-slate-500"
                }`}
              >
                {label}
              </button>
      ...
      {a?.tradeSetup && (
        <MemoForexTradeSetupPanel symbol={symbol} setup={a.tradeSetup as TradeSetupData} />
      )}
      ...
      {(mtf || fx || macro || analyst) && (
        <div className="space-y-3">
          ...
          <div className={`space-y-3 ${moreOpen ? "block" : "hidden lg:block"}`}>
            {analyst?.marketSummary && (
              <div className="panel space-y-2 p-3 text-xs text-slate-300 sm:p-4">
                ...
                <p className="line-clamp-4 lg:line-clamp-none">{analyst.marketSummary}</p>
                <div className="grid gap-2 sm:grid-cols-2">
                  <div className="rounded border border-emerald-900/40 bg-emerald-500/5 p-2">
                    <div className="text-[10px] font-semibold text-emerald-300">Kịch bản tăng</div>
                    <p className="mt-1 text-[10px]">{analyst.bullCase}</p>
                  </div>
                  <div className="rounded border border-rose-900/40 bg-rose-500/5 p-2">
                    <div className="text-[10px] font-semibold text-rose-300">Kịch bản giảm</div>
              ...
              <div className="panel p-3 sm:p-4">
                ...
                <div className="grid grid-cols-3 gap-1.5 sm:grid-cols-5 sm:gap-2">
                  {mtf.frames.map((fr: any) => (
                    <div key={fr.timeframe} className="rounded border border-slate-800 p-1.5 text-center text-[10px] sm:p-2 sm:text-xs">
            ...
            {fx && (
              <div className="grid grid-cols-1 gap-2 sm:grid-cols-3 sm:gap-3">
                <div className="panel p-3 text-xs">
                  ...
                  <div className="text-base text-[#00d4ff] sm:text-lg">{fx.session.label}</div>
                  <div className="text-slate-400">Biến động {fx.session.volatility} · Thanh khoản {fx.session.liquidity}</div>
                </div>
                <div className="panel p-3 text-xs">
              ...
              <div className="panel p-3 sm:p-4">
                ...
                <div className="flex gap-2 overflow-x-auto pb-1">
                  {macro.upcoming.slice(0, 6).map((ev: any) => (
                    ...
                    >
                      <div className="text-slate-500">{ev.flag} {ev.impact}</div>
                      <div className="line-clamp-2 font-semibold text-white">{ev.title}</div>
