...
interface Detail {
  pair: { symbol: string; name: string; category: string; baseCurrency: string; quoteCurrency: string };
  price: {
    price: number; bid: number | null; ask: number | null;
    change: number | null; changePercent: number | null; source: string; timestamp: string;
  } | null;
}

interface Analysis {
  recommendation: "BUY" | "SELL" | "NEUTRAL";
  entryPrice: number; stopLoss: number | null; takeProfit: number | null; confidence: number;
  reasons: string[]; indicators: Record<string, unknown>;
...
interface Bundle {
  pair: Detail["pair"]; price: Detail["price"]; bars: Bar[];
  timeframe: string; source: string; analysis: Analysis | null;
}
...
export default function ForexDetail() {
  ...
  useEffect(() => {
    ...
    api<Bundle>(`/forex/${symbol}/bundle?timeframe=${tf}&limit=300`)
      .then((env) => { if (!cancelled) { setBundle(env.data); setBundleLoading(false); } })
      .catch((err) => {
        if (!cancelled) {
  ...
  return (
    <div className="space-y-5">
      <Link href="/forex" className="text-xs text-[#00d4ff]">← Forex market</Link>

      <div className="panel p-4 flex flex-wrap items-center gap-4">
        <div className="h-12 w-12 rounded-full bg-[#00d4ff]/15 flex items-center justify-center text-lg font-black text-[#00d4ff]">FX</div>
        <div>
          ...
          <div className="text-[10px] text-slate-500">{p?.source ?? bundle?.source ?? "multi-source"} · Asia/Ho_Chi_Minh</div>
        </div>
        <div className="sm:ml-auto">
          <div className="text-3xl font-black text-white font-mono">{fmtNum(p?.price, p?.price && p.price > 1000 ? 2 : 5)}</div>
          <div className={`text-right font-bold ${changeColor(p?.changePercent)}`}>{fmtPct(p?.changePercent)}</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
            ...
            <div className="flex gap-1 overflow-x-auto">
              {TFS.map((x) => (
                <button key={x} onClick={() => setTf(x)}
                  className={`min-h-9 rounded px-3 text-xs ${tf === x ? "bg-[#00d4ff] text-[#0A2540]" : "bg-slate-800 text-slate-400"}`}>{x}</button>
              ))}
            </div>
          </div>
          {bars && bars.length > 0 ? (
            <CandleChart bars={bars} height={400} />
          ) : (
            <div className="h-96 flex items-center justify-center text-slate-500">
              {bundleError ? `Lỗi tải chart: ${bundleError}` : bundleLoading ? "Đang tải chart (race multi-source)..." : "Không có dữ liệu OHLCV"}
            </div>
          )}
        </div>
        ...
        <div className={`panel border p-4 ${style}`}>
          ...
          <div className="mt-4 grid grid-cols-2 gap-y-2 text-xs">
            <span className="opacity-60">Entry</span><span className="text-right font-mono">{fmtNum(a?.entryPrice, 5)}</span>
            <span className="opacity-60">Stop Loss</span><span className="text-right font-mono">{fmtNum(a?.stopLoss, 5)}</span>
            <span className="opacity-60">Take Profit</span><span className="text-right font-mono">{fmtNum(a?.takeProfit, 5)}</span>
          </div>
          <ul className="mt-4 text-xs space-y-1">{a?.reasons.slice(0, 6).map((x, i) => (<li key={i}>• {x}</li>))}</ul>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="panel p-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
            {a && Object.entries(a.indicators).filter(([, v]) => typeof v === "number" || v === null).map(([k, v]) => (
              <div key={k} className="rounded bg-slate-900/40 p-2 text-xs">
                <div className="text-slate-500">{k}</div>
                <div className="font-mono text-white mt-1">{typeof v === "number" ? fmtNum(v, 5) : "—"}</div>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="space-y-2 text-xs">
            {[...(a?.chartPatterns ?? []), ...(a?.candlestickPatterns ?? [])].slice(0, 8).map((x, i) => (
              <div key={i} className="flex justify-between rounded bg-slate-900/30 p-2">
                <span>{x.nameVi}</span>
                <span className={x.type === "bullish" ? "text-emerald-400" : x.type === "bearish" ? "text-rose-400" : "text-amber-400"}>
