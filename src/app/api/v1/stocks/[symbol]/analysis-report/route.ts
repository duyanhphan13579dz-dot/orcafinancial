...
import { getHistory, getNews } from "@/lib/market";
...
export async function GET(req: NextRequest, ctx: { params: Promise<{ symbol: string }> }) {
  ...
  try {
    ...
    const [profile, quarters, history, newsPayload] = await Promise.all([
      getProfile(symbol),
    ...
    const pdf = await renderStockAnalysisPdf({ symbol, generatedAt, profile, quarters, technical, health, forecast, risk, news, backtest, source: `${history.source} + sector-synthetic-v1 fallback + RSS news`, dataConfidence });
