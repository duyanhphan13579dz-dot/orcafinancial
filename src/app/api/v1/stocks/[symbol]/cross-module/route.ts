...
export async function GET(req: NextRequest, ctx: { params: Promise<{ symbol: string }> }) {
  ...
  try {
    ...
    return ok(context, { source: [...market.quality.sources, "commodity-impact", fxUsdVnd != null ? "exchange-rates" : "fx-unavailable"].join("+"), confidence: context.dataConfidence, stale: market.quality.stale, partial: context.missingModules.length > 0 }, { cacheSeconds: 60 });
  } catch (err) {
