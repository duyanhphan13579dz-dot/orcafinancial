...
interface ThesisPayload {
  thesis: ReturnType<typeof buildInvestmentThesis>;
  business: ReturnType<typeof buildBusinessIntelligence>;
  crossModule: ReturnType<typeof buildCrossModuleContext>;
  causalChains: ReturnType<typeof buildCrossModuleContext>["causalChains"];
  dataAsOf: { price: number | null; financial: string | null; generatedAt: string };
  source: string;
}
...
export async function GET(req: NextRequest, ctx: { params: Promise<{ symbol: string }> }) {
  ...
  try {
    ...
    const cached = await cachedStockPayload<ThesisPayload>({
      key: stockCacheKey("investment-thesis", symbol),
      ttlMs: 5 * 60_000,
      ...
      loader: async () => {
        ...
        if (history.bars.length < 30) throw new Error(`Insufficient history for ${symbol}`);
        ...
        return { thesis, business, crossModule, causalChains: crossModule.causalChains, dataAsOf: { price: history.bars.at(-1)?.time ?? null, financial: quarters[0]?.period ?? null, generatedAt: new Date().toISOString() }, source: history.source };
      },
    });
    const { thesis, business, crossModule, causalChains, dataAsOf, source } = cached.value;
    return ok({ thesis, business, crossModule, causalChains, dataAsOf }, { source: `${source}+market-overview+commodity-impact+sector-benchmark`, confidence: thesis.dataConfidence, partial: thesis.stance === "insufficient_data", disclaimer: thesis.disclosure, cache: cached.cache }, { cacheSeconds: 300 });
  } catch (err) {
