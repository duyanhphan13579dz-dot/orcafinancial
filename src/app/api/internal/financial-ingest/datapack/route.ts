...
export async function POST(request: NextRequest) {
  if (!process.env.FINANCIAL_AUDIT_SECRET && !process.env.CRON_SECRET) {
    return NextResponse.json({ ok: false, error: "Ingestion secret is not configured." }, { status: 503 });
  }
  ...
  const invalid = documents.some(
    (document) =>
      !ALLOWED_SOURCES.has(document.source) ||
      document.documentType !== "financial_statement" ||
      !document.documentUrl ||
      !document.sourceContent ||
      !Array.isArray(document.facts),
  );
  if (!documents.length || invalid) {
