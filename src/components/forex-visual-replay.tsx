...
export default function ForexVisualReplay({ data, onClose }: Props) {
  ...
  }, [playing, cursor, bars.length, speed]);
  ...
  const levels = data.setup ? { entry: data.setup.entry ?? null, stopLoss: data.setup.stopLoss ?? null, takeProfit: data.setup.takeProfit ?? null } : null;
