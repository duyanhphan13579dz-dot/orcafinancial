...
const REALTIME_STATUS_LABEL: Record<string, string> = { LIVE: "TRỰC TIẾP", DEGRADED: "SUY GIẢM", STALE: "DỮ LIỆU CŨ" };
...
function StockTooltip({ item, close }: { item: Item; close: () => void }) {
  ...
  return (
    <div className="fixed z-[80] inset-x-3 bottom-24 mx-auto w-[min(92vw,360px)] rounded-xl border border-slate-700 bg-slate-900/95 p-4 shadow-2xl backdrop-blur md:absolute md:inset-auto md:right-2 md:top-10 md:bottom-auto md:mx-0">
      ...
      {bars.length > 1 && <div className="mt-2 rounded-lg border border-slate-800 bg-slate-950/50 p-2"><div className="flex items-center gap-2"><button type="button" onClick={() => setPlaying((value) => !value)} className="rounded bg-[#00d4ff]/15 px-2 py-1 text-[10px] font-bold text-[#00d4ff]">{playing ? "Tạm dừng" : "Phát"}</button><input aria-label="Dòng thời gian phát lại" type="range" min={0} max={bars.length - 1} value={Math.min(playbackIndex, bars.length - 1)} onChange={(event) => { setPlaying(false); setPlaybackIndex(Number(event.target.value)); }} className="h-1 flex-1 accent-cyan-400" /><select aria-label="Tốc độ phát lại" value={speed} onChange={(event) => setSpeed(Number(event.target.value))} className="rounded bg-slate-800 px-1 py-1 text-[10px] text-slate-300"><option value={1}>1×</option><option value={2}>2×</option><option value={4}>4×</option></select></div><div className="mt-1 flex justify-between text-[9px] text-slate-500"><span>Phát lại</span><span>{new Date((bars[Math.min(playbackIndex, bars.length - 1)]?.time ?? 0) * 1000).toLocaleDateString("vi-VN")} · {fmtNum(bars[Math.min(playbackIndex, bars.length - 1)]?.close ?? null)}</span></div></div>}
      <div className="mt-3 rounded-lg border border-slate-800 bg-slate-950/40 p-3"><div className="mb-2 text-[10px] uppercase tracking-widest text-[#00d4ff]">Phân tích cổ phiếu</div><div className="grid grid-cols-2 gap-2 text-[10px]"><span className="text-slate-500">Tín hiệu</span><span className="text-right font-semibold text-white">{intelligence.analysis?.recommendation ?? "Đang phân tích"}</span><span className="text-slate-500">Điểm</span><span className="text-right font-mono text-slate-300">{intelligence.analysis?.score ?? "—"}</span><span className="text-slate-500">Kỹ thuật</span><span className="truncate text-right text-slate-300">{intelligence.technical?.chartPatterns?.[0]?.nameVi ?? intelligence.technical?.candlestickPatterns?.[0]?.nameVi ?? "Chưa có mẫu"}</span><span className="text-slate-500">Phân tích cơ bản</span><span className="text-right text-slate-300">ROE {intelligence.fundamental?.roe != null ? `${intelligence.fundamental.roe.toFixed(1)}%` : "—"} · P/E {intelligence.fundamental?.valuation?.pe ?? "—"}</span></div>{(intelligence.analysis?.reasons?.[0] || intelligence.news?.items?.[0]?.title) && <p className="mt-2 line-clamp-2 text-[10px] leading-4 text-slate-500">{intelligence.analysis?.reasons?.[0] ?? intelligence.news?.items?.[0]?.title}</p>}</div>
      <div className="mt-2 flex justify-between text-[10px] text-slate-500"><span>{item.isStale ? `Dữ liệu cũ ${item.ageSeconds ?? "—"}s` : "Dữ liệu mới"}</span><span>{item.source ?? "không rõ"} · {item.confidence != null ? `${Math.round(item.confidence * 100)}%` : "—"}</span></div>
      <Link href={`/stocks/${item.symbol}`} className="mt-3 flex min-h-11 items-center justify-center rounded-lg bg-[#00d4ff] text-xs font-bold text-[#0A2540] hover:brightness-110">Mở Phân tích cổ phiếu →</Link>
    </div>
  );
}
...
export function StockHeatmap({ compact = false }: { compact?: boolean }) {
  ...
  const marketBiasLabel = marketBias === "Bullish" ? "Tăng" : marketBias === "Bearish" ? "Giảm" : "Phân hóa";
  ...
  return (
    <section className="relative space-y-3">
        ...
        <div className="mr-auto flex items-center gap-2">
          ...
          {dataQuality && <span className="hidden text-[10px] text-slate-600 lg:inline">· {dataQuality.validQuoteCount ?? 0}/{dataQuality.universeCount ?? 0} mã · dữ liệu cũ {dataQuality.staleCount ?? 0}</span>}
          {realtime && <span className={`rounded px-1.5 py-0.5 text-[9px] font-bold ${realtime.status === "LIVE" ? "bg-emerald-400/15 text-emerald-300" : realtime.status === "DEGRADED" ? "bg-amber-400/15 text-amber-300" : "bg-rose-400/15 text-rose-300"}`}>{REALTIME_STATUS_LABEL[realtime.status ?? "STALE"] ?? realtime.status ?? "DỮ LIỆU CŨ"}{realtime.ageSeconds != null ? ` · ${realtime.ageSeconds}s` : ""}</span>}
        </div>
      ...
      {!compact && (
        <section className="grid gap-3 lg:grid-cols-[1.15fr_.85fr]">
          <div className="panel p-4">
            <div className="mb-3 flex items-center justify-between"><div><div className="text-[10px] uppercase tracking-[.25em] text-[#00d4ff]">THÔNG TIN THỊ TRƯỜNG</div><h2 className="mt-1 text-sm font-bold text-white">Độ rộng & luân chuyển ngành</h2></div><span className={`rounded-full px-2 py-1 text-[10px] font-bold ${marketBias === "Bullish" ? "bg-emerald-400/15 text-emerald-300" : marketBias === "Bearish" ? "bg-rose-400/15 text-rose-300" : "bg-amber-400/15 text-amber-300"}`}>{marketBiasLabel}</span></div>
          ...
          <div className="panel p-4"><div className="text-[10px] uppercase tracking-[.25em] text-[#00d4ff]">GÓC NHÌN AI ORCA</div><p className="mt-3 text-sm leading-6 text-slate-300">{aiInsight.insight ?? <>Dòng tiền đang tập trung vào nhóm <strong className="text-white">{sectorIntel[0]?.name ?? "—"}</strong>{sectorIntel[0]?.averageChange != null ? ` (${fmtPct(sectorIntel[0].averageChange)} bình quân)` : ""}. {sectorIntel[0]?.advancing ?? 0} mã tăng trong nhóm dẫn đầu.</>}</p><p className="mt-2 text-xs leading-5 text-slate-500">Nguồn: {aiInsight.provider ?? "bộ quy tắc"} · Trạng thái: <span className="font-semibold text-slate-300">{marketStatus === "TRADING" ? "Trong phiên" : STATUS_LABEL[marketStatus] ?? marketStatus}</span></p></div>
        </section>
      )}
