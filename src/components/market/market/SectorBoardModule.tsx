    ...
    <div className="space-y-5">
      <header className="panel overflow-hidden p-4 md:p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <div className="font-mono text-[10px] uppercase tracking-[0.2em] text-cyan-400">PHÂN TÍCH NGÀNH</div>
            <h1 className="mt-1 font-display text-2xl font-extrabold text-white">Bảng thị trường theo ngành</h1>
      ...
      {loading && !snapshot && <div className="panel p-8 text-center text-sm text-slate-400">Đang đồng bộ dữ liệu ngành…</div>}
      {error && <div className="panel border-rose-700 bg-rose-950/30 p-4 text-sm text-rose-300">Không lấy được dữ liệu ngành mới: {error}</div>}
      {snapshot && <>
        ...
        <section className="panel p-4">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div>
              <div className="font-mono text-[10px] uppercase tracking-[0.2em] text-cyan-400">TÓM TẮT NGÀNH</div>
              <h2 className="mt-1 font-display text-lg font-bold text-white">Tín hiệu xoay vòng</h2>
            </div>
            <span className="text-[10px] text-slate-500">Cập nhật {new Date(snapshot.generatedAt).toLocaleTimeString("vi-VN")}</span>
          </div>
          <div className="mt-3 grid gap-2 md:grid-cols-2">
            {[...snapshot.sectors].sort((a, b) => (b.strength ?? -1) - (a.strength ?? -1)).slice(0, 4).map((sector) => <div key={sector.id} className="flex items-center justify-between rounded-md border border-white/5 bg-[#091d34]/70 px-3 py-2 text-xs"><span className="text-slate-300">{sector.label}</span><span className="font-mono text-cyan-300">Sức mạnh {sector.strength ?? "—"}</span><span className="font-mono text-slate-500">{sector.stocks.length} mã</span></div>)}
          </div>
        </section>
      </>}
