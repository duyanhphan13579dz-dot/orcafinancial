...
export function StockMicrostructurePanel({ orderBook, foreignFlow }: Props) {
  ...
  const maxDepth = Math.max(1, ...orderBook.bids.map((row) => row.volume), ...orderBook.asks.map((row) => row.volume));
  const buy = foreignFlow.buyValue ?? 0;
  const sell = foreignFlow.sellValue ?? 0;
  const flowTotal = buy + sell;
  const buyShare = flowTotal ? buy / flowTotal * 100 : 50;
  return <div className="grid gap-4 xl:grid-cols-2">
    <section className="panel overflow-hidden">
      ...
      {unavailable ? <div className="p-5 text-xs text-slate-500">Sổ lệnh VNDirect / Vietstock đang cập nhật. Hệ thống sẽ tự cập nhật khi luồng dữ liệu được kết nối.</div> : <>
        ...
        <div className="px-4 pb-3"><div className="mb-2 flex items-end justify-center gap-1 border-b border-slate-800 pb-2"><div className="flex w-1/2 flex-row-reverse items-end gap-1">{orderBook.bids.map((row, i) => <div key={`bv${i}`} className="flex min-w-0 flex-1 flex-col items-center gap-1" title={`Mua ${row.price}: ${qty(row.volume)} cổ phiếu`}><div className="w-full rounded-t bg-emerald-500/60" style={{ height: `${Math.max(8, row.volume / maxDepth * 54)}px` }} /><span className="text-[9px] text-emerald-300">{row.price.toLocaleString("vi-VN")}</span></div>)}</div><div className="h-2 w-px bg-slate-700" /><div className="flex w-1/2 items-end gap-1">{orderBook.asks.map((row, i) => <div key={`av${i}`} className="flex min-w-0 flex-1 flex-col items-center gap-1" title={`Bán ${row.price}: ${qty(row.volume)} cổ phiếu`}><div className="w-full rounded-t bg-rose-500/60" style={{ height: `${Math.max(8, row.volume / maxDepth * 54)}px` }} /><span className="text-[9px] text-rose-300">{row.price.toLocaleString("vi-VN")}</span></div>)}</div></div><div className="grid grid-cols-2 gap-3"><div className="space-y-1">{orderBook.bids.map((row, i) => <div key={`b${i}`} className="flex justify-between rounded bg-emerald-500/[.06] px-2 py-1 text-[11px]"><span className="text-emerald-300">{row.price.toLocaleString("vi-VN")}</span><span className="text-slate-400">{qty(row.volume)}</span></div>)}</div><div className="space-y-1">{orderBook.asks.map((row, i) => <div key={`a${i}`} className="flex justify-between rounded bg-rose-500/[.06] px-2 py-1 text-[11px]"><span className="text-rose-300">{row.price.toLocaleString("vi-VN")}</span><span className="text-slate-400">{qty(row.volume)}</span></div>)}</div></div></div>
    ...
    <section className="panel overflow-hidden">
      ...
      {foreignFlow.status === "unavailable" ? <div className="p-5 text-xs text-slate-500">Dữ liệu giao dịch khối ngoại VNDirect / Vietstock đang cập nhật.</div> : <div className="p-4"><div className="grid grid-cols-3 gap-2 text-center"><div className="rounded-lg bg-emerald-500/[.08] p-3"><div className="text-[10px] text-slate-500">Mua vào</div><strong className="mt-1 block text-sm text-emerald-400">{money(foreignFlow.buyValue)}</strong></div><div className="rounded-lg bg-rose-500/[.08] p-3"><div className="text-[10px] text-slate-500">Bán ra</div><strong className="mt-1 block text-sm text-rose-400">{money(foreignFlow.sellValue)}</strong></div><div className="rounded-lg bg-slate-800/60 p-3"><div className="text-[10px] text-slate-500">Mua/bán ròng</div><strong className={`mt-1 block text-sm ${(foreignFlow.netValue ?? 0) >= 0 ? "text-emerald-400" : "text-rose-400"}`}>{money(foreignFlow.netValue)}</strong></div></div><div className="mt-4"><div className="mb-1 flex justify-between text-[10px] text-slate-500"><span>Mua {buyShare.toFixed(0)}%</span><span>Bán {(100 - buyShare).toFixed(0)}%</span></div><div className="flex h-2 overflow-hidden rounded-full bg-slate-800"><div className="bg-emerald-500" style={{ width: `${buyShare}%` }} /><div className="bg-rose-500" style={{ width: `${100 - buyShare}%` }} /></div></div><div className="mt-4 grid grid-cols-2 gap-y-2 text-xs"><span className="text-slate-500">Khối lượng mua</span><span className="text-right text-slate-300">{qty(foreignFlow.buyVolume)} cp</span><span className="text-slate-500">Khối lượng bán</span><span className="text-right text-slate-300">{qty(foreignFlow.sellVolume)} cp</span><span className="text-slate-500">Room còn lại</span><span className="text-right text-slate-300">{foreignFlow.foreignRoomPct == null ? "—" : `${foreignFlow.foreignRoomPct}%`}</span></div><div className="mt-4 border-t border-slate-800 pt-2 text-[10px] text-slate-500">Cập nhật {timeLabel(foreignFlow.updatedAt)} · Đơn vị giá trị: tỷ đồng</div></div>}
    </section>
  </div>;
}
