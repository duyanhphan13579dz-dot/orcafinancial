    ...
    <section className="panel border border-[#00d4ff]/20 p-3.5 sm:p-4">
      ...
      >
        <div>
          ...
          <div className="mt-1 text-sm font-semibold text-white">Multi-coin research signal</div>
        </div>
        <div className="flex items-center gap-2">
          {data && <span className={`rounded-md border px-2 py-1 text-xs font-black ${signalClass(data.signal)}`}>{data.signal}</span>}
            ...
            <>
              <div className={`rounded-lg border p-3 ${signalClass(data.signal)}`}>
                <div className="flex flex-wrap items-end justify-between gap-2">
                  <div>
                    <div className="text-[10px] uppercase tracking-wide opacity-70">Signal định lượng</div>
                    <div className="mt-1 text-3xl font-black">{data.signal}</div>
                  </div>
                  <div className="text-right text-[10px] opacity-80">
                    <div>{data.state} · {data.group}</div>
                    <div>Data quality {data.dataQuality.score.toFixed(0)}/15</div>
                  </div>
                </div>
              </div>
              ...
              {candidate ? (
                <div className="mt-3 space-y-3">
                  <div className="grid grid-cols-2 gap-2 text-xs sm:grid-cols-4">
                    <div className="rounded bg-black/20 p-2"><div className="text-slate-500">Strategy</div><div className="font-semibold text-white">{candidate.strategy}</div></div>
                    <div className="rounded bg-black/20 p-2"><div className="text-slate-500">Score</div><div className="font-mono font-semibold text-white">{candidate.score.toFixed(1)}/100</div></div>
                    ...
                    <div className="rounded bg-black/20 p-2"><div className="text-slate-500">Paper risk</div><div className="font-mono font-semibold text-white">${fmt(candidate.risk.riskAmount)}</div></div>
                  </div>
                  <div className="grid grid-cols-3 gap-2 text-center text-xs">
                    <div><div className="text-slate-500">Entry</div><div className="font-mono text-white">{fmt(candidate.entry)}</div></div>
                ...
                <div className="mt-3 text-xs text-slate-400">Chưa có setup đạt điều kiện đồng thuận; hệ thống giữ trạng thái WAIT/WATCH_ONLY.</div>
              )}
              ...
              <div className="mt-3 text-[10px] text-slate-500">Paper/research only · Không gửi lệnh thật · Cần backtest và paper trade trước khi sử dụng.</div>
            </>
          )}
        </div>
      )}
