  ...
  return (
    <section className="panel space-y-3 p-3 sm:p-4" aria-label="Forex scalping decision cockpit">
      ...
      {result && <>
        ...
        {candidate ? <div className="rounded border border-slate-700 bg-slate-900/30 p-3 text-xs"><div className="mb-2 flex flex-wrap items-center justify-between gap-2"><span className="font-semibold text-white">{candidate.strategy} · {candidate.direction}</span><span className={candidate.state === "QUALIFIED" ? "text-emerald-300" : "text-amber-300"}>{candidate.state}</span></div><div className="grid grid-cols-2 gap-2 sm:grid-cols-3 2xl:grid-cols-5"><div className="min-w-0 rounded border border-slate-800 bg-slate-950/20 p-2"><span className="block text-[10px] text-slate-500">Entry</span><div className="mt-1 break-words font-mono tabular-nums text-white">{fmt(candidate.entry)}</div></div><div className="min-w-0 rounded border border-rose-900/40 bg-rose-500/5 p-2"><span className="block text-[10px] text-slate-500">SL</span><div className="mt-1 break-words font-mono tabular-nums text-rose-300">{fmt(candidate.stopLoss)}</div></div><div className="min-w-0 rounded border border-emerald-900/40 bg-emerald-500/5 p-2"><span className="block text-[10px] text-slate-500">TP1</span><div className="mt-1 break-words font-mono tabular-nums text-emerald-300">{fmt(candidate.takeProfit)}</div></div><div className="min-w-0 rounded border border-slate-800 bg-slate-950/20 p-2"><span className="block text-[10px] text-slate-500">Stop</span><div className="mt-1 break-words font-mono tabular-nums text-white">{fmt(candidate.stopDistancePips, 1)} pips</div></div><div className="min-w-0 rounded border border-slate-800 bg-slate-950/20 p-2"><span className="block text-[10px] text-slate-500">Paper lot / R:R</span><div className="mt-1 break-words font-mono tabular-nums text-white">{fmt(candidate.risk.lotSize, 2)} / {fmt(candidate.riskRewardAfterCosts, 2)}</div></div></div><div className="mt-2 text-slate-400">{candidate.reasons.join(" · ")}</div>{candidate.hardBlocks.length > 0 && <div className="mt-2 text-amber-300">Why WAIT: {candidate.hardBlocks.join(" · ")}</div>}<div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={() => void saveSetup()} disabled={!onSaveSetup || saving} className="min-h-9 rounded border border-[#00d4ff]/40 px-3 text-[10px] text-[#00d4ff] disabled:cursor-not-allowed disabled:opacity-50">{saving ? "Saving…" : saved ? "Saved to journal" : "Watch setup"}</button><button type="button" className="min-h-9 rounded border border-slate-700 px-3 text-[10px] text-slate-400">View gate details</button></div></div> : <div className="rounded border border-slate-800 p-3 text-xs text-slate-400">WAIT — no qualified setup in the current M15–M5–M1 window.</div>}
