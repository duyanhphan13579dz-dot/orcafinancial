...
export default function ForexScalpingBoard() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      catch (err) { if (active) setError(err instanceof Error ? err.message : "Scalping board unavailable"); }
  ...
  return <section className="panel space-y-3 p-3 sm:p-4" aria-label="Forex scalping board">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex items-center gap-2"><h2 className="font-semibold text-white">Forex Scalping Board</h2><span className="rounded border border-amber-700/50 bg-amber-500/10 px-1.5 py-0.5 text-[9px] text-amber-300">PAPER ONLY</span></div><p className="mt-0.5 text-[10px] text-slate-500">M15 context · M5 confirmation · M1 trigger · Biquote-first</p></div><div className="text-right text-[10px] text-slate-500">{data?.generatedAt ? `updated ${new Date(data.generatedAt).toLocaleTimeString()}` : loading ? "scanning…" : "—"}</div></div>
    <div className="grid grid-cols-3 gap-2 text-[10px]"><div className="rounded border border-slate-800 p-2"><span className="text-slate-500">Pairs</span><div className="mt-1 text-sm font-semibold text-white">{rows.length}</div></div><div className="rounded border border-emerald-900/40 bg-emerald-500/5 p-2"><span className="text-slate-500">Ready</span><div className="mt-1 text-sm font-semibold text-emerald-300">{ready}</div></div><div className="rounded border border-amber-900/40 bg-amber-500/5 p-2"><span className="text-slate-500">Watch</span><div className="mt-1 text-sm font-semibold text-amber-300">{watch}</div></div></div>
    {error && <div className="rounded border border-rose-800/50 bg-rose-500/5 p-2 text-xs text-rose-300">{error}</div>}
    {loading && !data ? <div className="h-24 animate-pulse rounded bg-slate-800/40" /> : <div className="overflow-x-auto"><table className="w-full min-w-[680px] text-left text-[11px]"><thead className="border-b border-slate-800 text-[10px] uppercase tracking-wider text-slate-500"><tr><th className="px-2 py-2">Pair</th><th className="px-2 py-2">Signal</th><th className="px-2 py-2">M15–M5–M1</th><th className="px-2 py-2">Score</th><th className="px-2 py-2">Risk</th><th className="px-2 py-2">Action</th></tr></thead><tbody>{rows.map((row) => <tr key={row.symbol} className="border-b border-slate-900/80"><td className="px-2 py-2"><Link href={`/forex/${row.symbol}`} className="font-semibold text-white hover:text-[#00d4ff]">{row.symbol}</Link><div className="text-[9px] text-slate-600">quality {row.dataQuality.score}/15</div></td><td className={`px-2 py-2 font-bold ${signalTone(row.signal)}`}>{row.signal}</td><td className="px-2 py-2"><span className={stateTone(row)}>{row.state}</span><div className="max-w-[210px] truncate text-[9px] text-slate-500">{row.bestCandidate?.strategy ?? "No candidate"}</div></td><td className="px-2 py-2 font-mono text-white">{row.bestCandidate?.score ?? "—"}</td><td className="px-2 py-2 text-slate-400">{row.bestCandidate ? `${row.bestCandidate.stopDistancePips.toFixed(1)}p · R:R ${row.bestCandidate.riskRewardAfterCosts.toFixed(2)}` : row.blockers[0] ?? "—"}</td><td className="px-2 py-2"><Link href={`/forex/${row.symbol}`} className="rounded border border-slate-700 px-2 py-1 text-[10px] text-slate-300 hover:border-[#00d4ff]/50 hover:text-[#00d4ff]">Open detail</Link></td></tr>)}</tbody></table></div>}
    <button type="button" onClick={() => setShowBlocked((value) => !value)} className="text-[10px] text-slate-500 hover:text-slate-300">{showBlocked ? "Hide halted pairs" : "Show halted pairs"}</button>
  </section>;
}
